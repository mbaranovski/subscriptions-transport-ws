...
var SubscriptionClient = (function () {
    ...
    SubscriptionClient.prototype.close = function (isForced, closedByUser) {
        ...
        if (this.workerClient !== null) {
            ...
            this.worker.postMessage({ type: WorkerTypes_1.EVENT_TYPES.ONCLOSE_REQUEST });
            this.workerClient = null;
    ...
    SubscriptionClient.prototype.sendMessageRaw = function (message) {
        switch (this.status) {
            case this.wsImpl.OPEN:
                ...
                this.worker.postMessage({ type: WorkerTypes_1.EVENT_TYPES.REQUEST, value: serializedMessage });
                break;
    ...
    SubscriptionClient.prototype.registerWWEvents = function () {
        var _this = this;
        this.worker.onmessage = function (event) {
            var handler = _this.getWWEventHandler(event.data.type);
            if (!handler)
                return console.error('No handler for msg from WW', event.data.type);
            handler(event.data.value);
        };
    };
    SubscriptionClient.prototype.registerWWEventHandlers = function () {
    };
    SubscriptionClient.prototype.wwConnect = function () {
        var _this = this;
        this.wwEventHandlers.set(WorkerTypes_1.EVENT_TYPES_SEND_WW.ONOPEN, function (value) { return __awaiter(_this, void 0, void 0, function () {
        ...
        this.wwEventHandlers.set(WorkerTypes_1.EVENT_TYPES_SEND_WW.ONCLOSE, function () {
            if (!_this.closedByUser) {
                _this.close(false, false);
            }
        });
        this.wwEventHandlers.set(WorkerTypes_1.EVENT_TYPES_SEND_WW.ONERROR, function (err) {
            _this.eventEmitter.emit('error', err);
        });
        this.wwEventHandlers.set(WorkerTypes_1.EVENT_TYPES_SEND_WW.ONMESSAGE, function (data) {
            if (Array.isArray(data)) {
                console.time("processing");
                data.forEach(function (data) { return _this.processReceivedData(data); });
    ...
    SubscriptionClient.prototype.processReceivedData = function (receivedData) {
        ...
        switch (parsedMessage.type) {
            ...
            case message_types_1.default.GQL_DATA:
                var parsedPayload_1 = !parsedMessage.payload.errors ?
                    parsedMessage.payload : __assign({}, parsedMessage.payload, { errors: this.formatErrors(parsedMessage.payload.errors) });
                var operation_1 = this.operations[opId];
                setTimeout(function () {
                    operation_1.handler(null, parsedPayload_1);
                });
                break;
